...
// also, then the offset/bias and scaling factor can be built in to the data
// type
...
class FailedToLoad : public std::runtime_error {
 public:
  FailedToLoad(const std::string& what_arg) : std::runtime_error(what_arg) {}
};

class BadPixelData : public std::runtime_error {
 public:
  BadPixelData(const std::string& what_arg) : std::runtime_error(what_arg) {}
};

// generateBlurKernel() throws UnsupportedKernelType exception
class UnsupportedKernelType : public std::runtime_error {
 public:
  UnsupportedKernelType(const std::string& what_arg)
...
bool saveTargaColorMatrix(const std::string& targaFilename,
...
void threshold(const ColorMatrix& source, ColorMatrix& dest,
               const Component& threshold = 128);
void desaturate(const ColorMatrix& source, ColorMatrix& dest, double amount);
void convolve(const ColorMatrix& source, ColorMatrix& dest,
              const KernelMatrix& kernel, const unsigned int offset = 0);
void sep_convolve(const ColorMatrix& source, ColorMatrix& dest,
                  const std::vector<double>& kernel,
                  const unsigned int offset = 0);
void drawSquare(ColorMatrix& dest, unsigned int centerX, unsigned int centerY,
...
ColorMatrix threshold(const ColorMatrix& source,
                      const Component& threshold = 128);
ColorMatrix desaturate(const ColorMatrix& source, double amount);
ColorMatrix convolve(const ColorMatrix& source, const KernelMatrix& kernel,
                     const unsigned int offset = 0);
ColorMatrix sep_convolve(const ColorMatrix& source,
                         const std::vector<double>& kernel,
                         const unsigned int offset = 0);
ColorMatrix sobel(const ColorMatrix& source);

// TODO : work in progress, needs to be a generic resizer that calls out to the
// more specific resizing functions...
ColorMatrix resize(const ColorMatrix& source, const unsigned int rows,
                   const unsigned int cols);
ColorMatrix resizeNearest(const ColorMatrix& source, const unsigned int rows,
                          const unsigned int cols);
ColorMatrix resizeBilinear(const ColorMatrix& source, const unsigned int rows,
                           const unsigned int cols);
...
std::vector<double> generate1dMitchell(const int radius,
...
// Clamp a double value into the unsigned char range needed for a Component
// value
Component clampf(double value, const Component& min = 0,
                 const Component& max = 255);
